...
type ImgService interface {
	FormatFromExtension(ext string) (files.Format, error)
	Resize(ctx context.Context, in io.Reader, width, height int, out io.Writer, options ...files.Option) error
}
...
func handleImagePreview(
	...
	if errors.Is(err, files.ErrUnsupportedFormat) || format == files.FormatGif {
		return rawFileHandler(w, r, file)
	}
	if err != nil {
		return errToStatus(err), err
...
func createPreview(imgSvc ImgService, fileCache FileCache,
	...
	case PreviewSizeBig:
		width = 1080
		height = 1080
		options = append(options, files.WithMode(files.ResizeModeFit), files.WithQuality(files.QualityMedium))
	case PreviewSizeThumb:
		width = 256
		height = 256
		options = append(options, files.WithMode(files.ResizeModeFill), files.WithQuality(files.QualityLow), files.WithFormat(files.FormatJpeg))
	default:
		return nil, files.ErrUnsupportedFormat
	}
