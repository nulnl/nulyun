...
func init() {
	...
	rootCmd.SetVersionTemplate("Nul Yun version {{printf \"%s\" .Version}}\n")

	// Flags available across the whole program
	persistent := rootCmd.PersistentFlags()
	persistent.StringP("config", "c", "", "config file path")
	persistent.StringP("database", "d", "./nulyun.db", "database path")
...
var rootCmd = &cobra.Command{
	Use:   "nulyun",
	Short: "A stylish web-based nul yun",
	Long: `Nul Yun CLI lets you create the database to use with Nul Yun,
manage your users and all the configurations without accessing the
web interface.

If you've never run Nul Yun, you'll need to have a database for
...
If "--config" is not specified, Nul Yun will look for a configuration
file named .nulyun.{json, toml, yaml, yml} in the following directories:
...
Also, if the database path doesn't exist, Nul Yun will enter into
...
user created with the credentials from options "username" and "password".`,
	RunE: withViperAndStore(func(_ *cobra.Command, _ []string, v *viper.Viper, st *store) error {
		...
		imageService := files.NewImage(imgWorkersCount)
		...
		if err := setTOTPEncryptionKey(set, st.Storage); err != nil {
			return err
		}
...
func setTOTPEncryptionKey(set *settings.Settings, store *storage.Storage) error {
	// 1. If key is already set (len 32), use it
	if len(set.TOTPEncryptionKey) == 32 {
		return nil
	}

	// 2. Generate one and save it
